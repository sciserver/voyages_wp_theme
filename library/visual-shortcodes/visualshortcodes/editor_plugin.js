    ...
    tinymce.create( 'tinymce.plugins.visualShortcodes', {
        ...
        _doScImage: function( co ){
            ...
            return co.replace( t.regex, function(a,b,c){
            		// add custom CSS classes for visual editor
								var classes = b.replace('_', '-');

                return '<p class="wproto-visual-shortcode" style="background: #eee url('+t.shortcodes[b].image+') center center no-repeat"><img src="'+t.shortcodes[b].image+'" id="vscImage'+(t.counter++)+'-'+b+'" class="' + classes + ' mceItem jpbVisualShortcode" title="' + b + tinymce.DOM.encode(c) + '" />';
            });
        },
