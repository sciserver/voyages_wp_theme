...
<h2><?php _e('Catalog section', 'wproto'); ?></h2>
...
<table class="form-table wproto-form-table">
	...
	<tr>
		...
		?>
		<th><p><?php _e( 'Display items from categories', 'wproto' ); ?>:</p></th>
	...
	<tr class="wproto-display-posts-control-block" <?php echo $display_type == 'all' ? 'style="display: none;"' : ''; ?>>
		...
		<td>
			...
			<div class="wproto-posts-categories-chooser-content">
				...
				<?php
					$tax = get_terms( 'wproto_catalog_category', array(
						'hide_empty' => 0
					));
				?>
	...
	<tr>
		...
		<td>
			...
			<input id="wproto_section_data-limit" name="wproto_section_data[limit]" type="number" min="1" max="100" value="<?php echo isset( $data['wproto_section_data']['limit'] ) ? $data['wproto_section_data']['limit'] : 8; ?>" />
