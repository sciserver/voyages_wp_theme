...
<h2><?php _e('Benefits section', 'wproto'); ?></h2>
...
<table class="form-table wproto-form-table">
	...
	<tr>
		...
		?>
		<th><p><?php _e( 'Display benefits from categories', 'wproto' ); ?>:</p></th>
	...
	<tr class="wproto-display-posts-control-block" <?php echo $display_type == 'all' ? 'style="display: none;"' : ''; ?>>
		...
		<td>
			...
			<div class="wproto-posts-categories-chooser-content">
				...
				<?php
					$tax = get_terms( 'wproto_benefits_category', array(
						'hide_empty' => 0
					));
				?>
	...
	<tr>
		<th><label for="wproto_section_data-limit"><?php _e( 'Items limit', 'wproto' ); ?>:</label></th>
		<td>
		
			<input id="wproto_section_data-limit" name="wproto_section_data[limit]" type="number" min="1" max="100" value="<?php echo isset( $data['wproto_section_data']['limit'] ) ? $data['wproto_section_data']['limit'] : 4; ?>" />
	...
	<tr>
		<th><label for="wproto-section-editor-after-text"><?php _e( 'Text after section content (optional)', 'wproto' ); ?>:</label></th>
		<td>
