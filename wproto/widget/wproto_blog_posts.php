<?php
	...
	add_action( 'widgets_init', 'register_wpl_galaxy_wp_blog_posts_widget' );
	
	function register_wpl_galaxy_wp_blog_posts_widget() {
		register_widget( 'wpl_galaxy_wp_blog_posts_widget' );
	}
	
	class wpl_galaxy_wp_blog_posts_widget extends WP_Widget {
		
		function __construct() {
			$widget_ops = array( 'classname' => 'wproto-blog-posts-widget', 'description' => __('A widget that displays the blog posts. ', 'wproto') );
		...
		function widget( $args, $instance ) {
			...
			$wpl_galaxy_wp->view->load_partial( 'widgets/blog_posts', $data );
		}
		...
		function form( $instance ) {
			...
			<p>
				<input class="checkbox" value="1" type="checkbox" <?php checked( $instance['sticky_only'] ); ?> id="<?php echo $this->get_field_id( 'sticky_only' ); ?>" name="<?php echo $this->get_field_name( 'sticky_only' ); ?>" />
				<label for="<?php echo $this->get_field_id( 'sticky_only' ); ?>"><?php _e('Sticky only', 'wproto'); ?></label>  
			</p>
			<p>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'posts_count' ); ?>"><?php _e('Number of posts to display:', 'wproto'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'order_by' ); ?>"><?php _e('Order posts by:', 'wproto'); ?></label>
