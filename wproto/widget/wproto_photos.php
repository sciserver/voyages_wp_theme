<?php
	...
	add_action( 'widgets_init', 'register_wpl_galaxy_wp_photos_widget' );
	
	function register_wpl_galaxy_wp_photos_widget() {
		register_widget( 'wpl_galaxy_wp_photos_widget' );
	}
	
	class wpl_galaxy_wp_photos_widget extends WP_Widget {
		
		function __construct() {
			$widget_ops = array( 'classname' => 'wproto-photos-widget', 'description' => __('A widget that displays the photos. ', 'wproto') );
		
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wproto-photos-widget' );
		
			//$this->WP_Widget( 'wproto-photos-widget', __( '&laquo;Galaxy&raquo; Photos', 'wproto' ), $widget_ops, $control_ops );
			parent::__construct( 'wproto-photos-widget', __( '&laquo;Galaxy&raquo; Photos', 'wproto' ), $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			$wpl_galaxy_wp->view->load_partial( 'widgets/photos', $data );
		}
		...
		function form( $instance ) {
			...
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id( 'link_to' ); ?>" name="<?php echo $this->get_field_name( 'link_to' ); ?>">
					<option <?php echo $instance['link_to'] == 'file' ? 'selected="selected"' : ''; ?> value="file"><?php _e( 'File (LightBox)', 'wproto' ); ?></option>
