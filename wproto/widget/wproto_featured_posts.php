<?php
	...
	add_action( 'widgets_init', 'register_wpl_galaxy_wp_featured_posts_widget' );
	
	function register_wpl_galaxy_wp_featured_posts_widget() {
		register_widget( 'wpl_galaxy_wp_featured_posts_widget' );
	}
	
	class wpl_galaxy_wp_featured_posts_widget extends WP_Widget {
		
		function __construct() {
			$widget_ops = array( 'classname' => 'wproto-featured-posts-widget', 'description' => __('A widget that displays the featured posts. ', 'wproto') );
		
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wproto-featured-posts-widget' );
		
			//$this->WP_Widget( 'wproto-featured-posts-widget', __( '&laquo;Galaxy&raquo; Featured Posts', 'wproto' ), $widget_ops, $control_ops );
			parent::__construct( 'wproto-featured-posts-widget', __( '&laquo;Galaxy&raquo; Featured Posts', 'wproto' ), $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			$data['posts'] = $wpl_galaxy_wp->model->post->get_featured_posts( $instance['post_type'], $instance['count'] );
			
			$data['instance'] = $instance;
			$data['args'] = $args;

			$wpl_galaxy_wp->view->load_partial( 'widgets/featured_posts', $data );
		}
